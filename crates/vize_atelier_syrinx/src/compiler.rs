...
use vize_atelier_core::{
    ElementType, ExpressionNode, ForNode, IfNode, Namespace, PropNode, SimpleExpressionNode,
...
use vize_atelier_vapor::{
    BlockIRNode, ForIRNode, IfIRNode, NegativeBranch, OperationNode, RootIRNode,
    VaporCompilerOptions, VaporTemplateAnnotations, compile_vapor_ir_with_template_syntax,
    generate_vapor_fragment_template,
};
...
use crate::model::{
    AcceptedValueKind, BindingKind, BindingSite, BranchDefinition, ComponentDefinition,
    ComponentPlanV1, HandlerSite, IfSite, KeyedListSite, ProtocolLimits, ProtocolVersion,
    SourceSpan, Stylesheet, SyrinxArtifacts, SyrinxManifestV1, SyrinxSourceMapV1,
    TemplateDefinition,
};
...
pub fn compile_syrinx(
    ...
    let mut state = CompileState::new(ir, &mut spans, scope_token.clone());
    ...
    for child in children {
        match child {
            TemplateChildNode::Element(element) => {
                if element.tag_type == ElementType::Component || element.tag.as_str() == "component"
                {
                    diagnostics.push(location_diagnostic(
                        ...
                        "Nested and dynamic components require NYM-765 and cannot render almost-correctly in v2.2.",
                        &element.loc,
                    ));
                } else if element.tag_type == ElementType::Slot {
                    diagnostics.push(location_diagnostic(
                        source,
                        filename,
                        block,
                        "SYRINX_UNSUPPORTED_SLOT",
                        "Slot outlets require NYM-765 scope composition and commit routing.",
                        &element.loc,
                    ));
                }
...
fn inspect_block(
    ...
    for operation in block.operation.iter() {
        inspect_operation(source, filename, template_loc, operation, ir, diagnostics);
    }
    for effect in block.effect.iter() {
        for operation in effect.operations.iter() {
            inspect_operation(source, filename, template_loc, operation, ir, diagnostics);
...
fn inspect_operation(
    ...
    let stub_location = SourceLocation::STUB;
    let unsupported = match operation {
        ...
        OperationNode::CreateComponent(node) => Some((
            "SYRINX_UNSUPPORTED_COMPONENT",
            "Nested components require NYM-765 scope composition and commit routing.",
            ir.control_source_map
                .get(&node.id)
                .unwrap_or(&stub_location),
        )),
        OperationNode::SlotOutlet(node) => Some((
            "SYRINX_UNSUPPORTED_SLOT",
            "Slot outlets require NYM-765 scope composition and commit routing.",
            ir.control_source_map
                .get(&node.id)
...
fn inspect_negative(
    ...
    match negative {
        Some(NegativeBranch::Block(block)) => {
            inspect_block(source, filename, template_loc, block, ir, diagnostics)
        }
